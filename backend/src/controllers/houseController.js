  ...
  try { 
    ...
    if (!isAdmin) { 
      ...
      houses = houses.map(house => { 
        ...
        if (houseObj._id.toString() !== assignedHouseId) { 
          delete houseObj.whatsappLink; 
          delete houseObj.whatsAppGroupLink; // Secondary protection clearance hook
        } 
        return houseObj; 
      }); 
    } 
...
const updateHouse = async (req, res) => { 
  try { 
    ...
    // Mirror the payload mapping inside your update sequence for uniform data integrity
