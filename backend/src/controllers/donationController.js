...
const submitDonation = async (req, res) => { 
  try { 
    ...
    const donation = await Donation.create({ 
      ...
      // FIXED LAYER: Explicitly binds both tracking parameters to satisfy your Mongoose validator rules
      memberId: activeMemberId,
      userId: activeMemberId,
...
const approveDonation = async (req, res) => { 
  try { 
    ...
    // Notify the member profile dashboard feed 
    await Notification.create({ 
      memberId: donation.memberId || donation.userId, 
      title: "Donation Approved", 
      message: "Your donation deposit has been successfully verified and approved. Thank you!", 
      type: "DonationApproval" 
    }); 
...
// FIXED FETCH ENGINE: Implements strictPopulate bypass flags across multiple potential model layout fields
const getDonations = async (req, res) => { 
  try { 
    const donations = await Donation.find() 
      .populate({ path: "memberId", select: "username email", options: { strictPopulate: false } }) 
      .populate({ path: "userId", select: "username email", options: { strictPopulate: false } }) 
      .populate({ path: "campaignId", select: "title status", options: { strictPopulate: false } })
      .sort({ createdAt: -1 });
